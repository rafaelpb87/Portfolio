...
month_order = {'Enero': 1, 'Febrero': 2, 'Marzo': 3, 'Abril': 4, 'Mayo': 5, 'Junio': 6, 'Julio': 7,
...
.chart-row { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 30px; }
.chart-container { flex: 0 0 49%; max-width: 49%; background: #fff; padding: 10px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); }
.chart-container > div { width: 100% !important; height: 400px; } /* Ensure consistent height and override Plotly width */
h1, h2 { margin-bottom: 20px; }
@media (max-width: 1200px) {
  .chart-container { flex: 0 0 100%; max-width: 100%; }
}
</style>
</head>
<body>
...
for proc in processes:
    ...
    for ind in indicators:
        ...
        fig.update_layout(xaxis_title='Fecha', yaxis_title='Valor', width=600, height=400)
        # add goal lines
        ...
        if not meta_row.empty:
            ...
            fig.add_hline(y=meta1, line_dash="dash", line_color="green", annotation_text="Meta1", annotation_position="right")
            if pd.notna(meta2) and meta2 != '':
                fig.add_hline(y=meta2, line_dash="dash", line_color="red", annotation_text="Meta2", annotation_position="right")
            colors = []
            for val in df_ind['Resultado Reportado']:
                if tendencia == 'Ascendente': colors.append('green' if val >= meta1 else 'red')
                elif tendencia == 'Descendente': colors.append('green' if val <= meta1 else 'red')
                elif tendencia == 'Rango' and pd.notna(meta2): colors.append('green' if meta1 <= val <= meta2 else 'red')
